...
const MOCK_ORDERS: Order[] = [
  {
    ...
    order_datetime: '2025-03-14T10:22:00Z',
  ...
  },
  {
    order_id: 1002,
    customer_id: 2,
    customer_name: 'Brian Okafor',
    order_datetime: '2025-03-14T11:05:00Z',
  ...
  },
  {
    order_id: 1003,
    customer_id: 3,
    customer_name: 'Clara Mendez',
    order_datetime: '2025-03-14T13:47:00Z',
  ...
  },
  {
    ...
    order_datetime: '2025-03-15T08:30:00Z',
  ...
  },
  {
    ...
    order_datetime: '2025-03-15T09:12:00Z',
    payment_method: 'credit_card',
    device_type: 'web',
    order_total: 374.95,
    risk_score: 0.68,
  ...
  },
  {
    ...
    order_datetime: '2025-03-15T14:00:00Z',
...
export default function AdminDashboard() {
  ...
  async function handleRunScoring() {
    ...
    try {
      ...
      showToast(
        'Scoring failed. Check backend logs for the Python scoring error.',
        'error'
      );
    } finally {
      setScoring(false);
        ...
        >
          {(['orders', 'queue'] as const).map((tab) => (
            ...
            >
              {tab === 'orders'
                ? 'Order History'
                : `Priority Queue${queue.length > 0 ? ` (${queue.length})` : ''}`}
            </button>
          ))}
        </div>
          ...
          <div>
            {queue.length === 0 ? (
              <div className="empty-state fade-in">
                ...
                <p>
                  No scoring results yet. Click <strong>Run Scoring</strong> to
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      Orders ranked by fraud probability — review highest risk
                      first.
                    </div>
                  </div>
                  <span
                  ...
                  >
                    Last run:{' '}
                    {fmtDate(queue[0]?.scored_at ?? new Date().toISOString())}
                  </span>
                </div>
                <table className="data-table">
                  <thead>
                        ...
                        <td>
                          <span
                            style={{
                              fontFamily: 'var(--font-mono)',
                              fontSize: 13,
                              color: riskColor(item.fraud_probability),
                              fontWeight: 600,
                            }}
                          >
